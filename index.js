...
/**
 * Функція `customEntries` повертає масив, що містить усі властивості об'єкта
 *  та їх значення у вигляді масивів.
...
/**
 * Функція `customObjectFromEntries` створює об'єкт з масиву записів,
 * додатково обробляючи значення властивостей.
...
function customObjectFromEntries(entries) {
  ...
  if (Array.isArray(entries) === true) {
    const newarr = entries.map(([key, value]) =>
      typeof key === "number" ? [key.toString(), key.toString()] : [key, value]
    );
...
function extendAndFreezeObject(obj1, obj2) {
  ...
  if (typeof obj1 === "object" && typeof obj2 === "object") {
    ...
    Object.freeze(obj3);
    if (Object.isFrozen(obj3) === true) {
      return obj3;
    } else `об'єкт не було "заморожено`;
  } else {
    return null;
  }
}
...
function filterObjectsByKey(arr, key) {
  ...
  if (Array.isArray(arr) === true) {
    const newarr = arr.filter((obj) =>
      obj.hasOwnProperty(key) === true ? obj : null
    );
    return newarr;
  } else {
    return [];
...
function getObjectValuesSum(obj) {
  ...
  if (typeof obj === "object") {
    ...
    const sum = value.reduce(
      (previousVla, currentVal) => previousVla + currentVal,
      0
    );
    return sum;
  } else 0;
...
function convertArrayToObj(arr) {
  ...
  // Проходимося по кожному підмасиву в масиві за допопмогою циклу for,
  //лічильник від нуля до довжини масиву
